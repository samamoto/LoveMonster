...
public class ThirdPersonCharacter : MonoBehaviour
{
    ...
    /// <summary>
    /// 壁蹴りの処理
    /// コライダーは壁に当たっている時に壁蹴りができる
    /// </summary>
    void OnCollisionStay(Collision collision)
    {
        if (!onGround && collision.contacts[0].normal.y < 0.1f) // 空中にいる、そして壁が傾いていないことが成立していないと壁蹴りはできない
        {
            if (jumpInput)
            {
