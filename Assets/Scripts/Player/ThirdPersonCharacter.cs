...
public class ThirdPersonCharacter : MonoBehaviour {
	...
	[SerializeField]
	public bool onGround; // Is the character on the ground
	...
	public float wallJumpFactor = 0.8f; // 壁蹴り時のジャンプ力減少値
	...
	private Vector3 m_GroundContactNormal;	//2018年01月10日 oyama add
	...
	void OnCollisionStay(Collision collision) {
		...
		if (!onGround && collision.contacts[0].normal.y < 0.1f) // 空中にいる、かつ壁が傾いていないことが成立していないと壁蹴りはできない
		{
			...
			if (jumpInput && GroundDistance > 0.2f) {
				//壁キックSE
				m_Audio.PlayOneShot((int)AudioList.SoundList_SE.SE_ActionJump); float dotDirection = Vector3.Dot(collision.contacts[0].normal, GetComponent<Rigidbody>().transform.forward);

				velocity = (collision.contacts[0].normal + GetComponent<Rigidbody>().transform.forward).normalized * airSpeed;

				//　プレイヤーは壁に直面した時は壁の法線方向に弾くようにJump
				if (dotDirection < 0f) {
					velocity = collision.contacts[0].normal * airSpeed;
				}

				//　プレーヤーの動きをスピードに足していくが、ジャンプ力を重ねないように
				velocity += GetComponent<Rigidbody>().velocity;
				velocity.y = jumpPower * wallJumpFactor;

				GetComponent<Rigidbody>().velocity = velocity;// 新しくvelocityを与える

				//　壁蹴ったら前進方向を計算する
				Vector3 targetDirection = (transform.position + GetComponent<Rigidbody>().velocity) - transform.position;
				targetDirection.y = 0; //　プレーヤーは Y 軸で回転するから０にする必要がある

				//　向きに合わせて回転を入れる
				Quaternion rotation = Quaternion.LookRotation(targetDirection);
				transform.rotation = rotation;

				GetComponent<PlayerManager>().isWallJumping = true;
				StartCoroutine(afterWallJump());
			}
		}
	}

	// coroutine to prevent set PlayerManagers isWallJumping bool, which is used to prevent
	// player from changing the direction the character is facing after wall jumping
	IEnumerator afterWallJump() {
		yield return new WaitForSeconds(wallJumpLockTime);
		GetComponent<PlayerManager>().isWallJumping = false;
	}
}
